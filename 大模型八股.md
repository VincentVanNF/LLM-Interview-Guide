...
- 将embedding为维度推广到d_model维度：
  ...
  - 其中 $m$ 代表当前token的序列位置, 两两一组使用相同的 $\theta$, $\theta$计算公式为 $\theta_i = \frac{1}{10000^{2i/d}}$
...
- **核心思路**：传统的 RoPE 扩展会把全体频率按固定比例 $s$ 统一“拉伸/压缩”（PI、NTK-aware 等），但推理时真实的序列长度是逐步增长的：第1步长度很短，之后才逐步接近目标长度。**Dynamic NTK** 的核心就是：**在每一次前向中根据“当前序列长度”动态更新缩放系数 $s$**，从而避免对短段落“过度拉伸”、对长段落“突然失真。

- 设模型原始预训练最大长度为 $L$，当前前向（或当前解码步）序列长度为$L_c$，$s$为频率的放缩系数，动态缩放使用
  $$
  s \;=\; \max\!\Bigl(1,\; \frac{L_c}{L}\Bigr)
  $$

- 直观理解：靠前的 token 仍使用“接近原网学到的**高频**细节”，**序列越长**才越“拉伸”**低频**，以保持远距离相对位置信息。

### YaRN 

- 短序列->高频系数；长序列->低频系数

- 在 RoPE 上**分段（by-parts）插值**以兼顾高/低频，再**给注意力加温度缩放**来稳定长程匹配；推理时可选配**Dynamic Scaling** 进一步缓解越界退化。短序列
  - NTK-by-parts（分段插值）：**by-parts** 进一步把频率（或等价的波长 $\lambda$）按区间处理：**短波长（高频）不插值，中间区间平滑过渡，长波长（低频）做线性插值**，这样既保住近邻分辨率，又能把相对长距离的信息拉开。
  - 注意力“温度”缩放（Attention Scaling）：$\text{softmax}\left(\frac{q^T k}{t\sqrt{|D|}}\right)$
  - 经验上，**随着扩展倍数$s$增大，适度“降温”能稳定困惑度**，LLaMA 系列经验公式：$\frac{1}{t} = 0.1 \ln(s) + 1$
...
- RoPE旋转位置编码:
  ...
  - 将这种变化用复数表示:
    ...
    **含义说明：**
    ...
    - $R_f(\cdot), R_g(\cdot)$：**幅度 / 模长**（非负实数），即 $|f|, |g|$。
    - $\Theta_f(\cdot), \Theta_g(\cdot)$：**相位 / 辐角**（弧度），即 $\arg(\cdot)$。
    - 指数项 $e^{i\Theta} = \cos\Theta + i\sin\Theta$ 表示相位对应的复指数。
    
    **从 $f$ 或 $g$ 得到幅度与相位：**
    $$
    R_f = \sqrt{a^2+b^2}, \quad 
    \Theta_f = \operatorname{atan2}(b,a),
    $$
    其中 $f = a + ib$，$g$ 同理。
    
  - ![image-20250813230313965](https://s2.loli.net/2025/08/13/gGyW4smBXdQA7IL.png)
  - ![image-20250813230258980](https://s2.loli.net/2025/08/13/BjKPDga5tdWJCEv.png)
  - 最终得到的结果说明: **该复数(即该函数变化)可以通过原向量旋转 $m\theta$** 得到，其中m为该token在序列中位置
  - 两两一个组合，根据位置m旋转一个角度。
  - **内积可以表示为 (m-n)相对位置的一个函数**
  - 绝对位置编码:两两一对，偶数位sin，奇数位cos。
    - 不能很好表示相对位置，扩展性不好
  
- KV cache
  - **推理的时候只会用到最后一个token的序列输入到词表映射层**，因此计算attention score时，不需要之前的Q值，只需要缓存之前序列的K,V值，即可获得最后一个序列经过attention 之后的值。
